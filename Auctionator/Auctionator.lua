...
function Atr_SB_BagUpdate()
    if (UnitAffectingCombat("player") or GetPlayerMapPosition("player") == nil) then
        return; -- Skip during zone transitions or combat
    end
...
function Atr_OnUpdate(self, elapsed)
    ...
    if (gAtr_dustCacheIndex > 0 and zc.periodic(self, "dust_lastUpdate", 0.2, elapsed)) then -- Increased from 0.1 to 0.2
