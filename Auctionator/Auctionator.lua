...
local function Atr_IsItemSellableOnAH(bag, slot, link, quality)
    ...
    -- Filter quest items unless they are Uncommon (green) or better
        ...
        for i = 1, num do
            ...
            if (t) then
                if ((ITEM_SOULBOUND and t:find(ITEM_SOULBOUND, 1, true)) or
                    (ITEM_BIND_ON_PICKUP and t:find(ITEM_BIND_ON_PICKUP, 1, true))) then
                    return false;
                end
            end
        end
...
local function Atr_SB_Item_OnClick(self)
    if (self.bagID and self.slotID) then
