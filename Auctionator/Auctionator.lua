	...
	elseif (Atr_IsAuctionatorTab(index)) then
		...
		if (index == Atr_FindTabIndex(SELL_TAB)) then
            ...
            if (AuctionatorScrollFrame) then
                AuctionatorScrollFrame:SetHeight(40);
            end
		else
			Atr_Hlist:Show();
...
-- Determine if an item can be listed on the AH (exclude BoP/Soulbound and grey quality)
local function Atr_IsItemSellableOnAH(bag, slot, link, quality)
    if (not link) then return false; end
    if (quality ~= nil and quality == 0) then return false; end  -- grey quality

    -- Fallback: if bag API didn't provide quality, ask item API and still filter greys
    if (quality == nil) then
        local _, _, itemQuality = GetItemInfo(link);
        if (itemQuality ~= nil and itemQuality == 0) then
            return false;
        end
        -- Final fallback: link color for Poor quality is 9d9d9d
    ...
    -- Do NOT filter based solely on container quest flags; some reagents are noisy here.
    -- We'll rely on tooltip text to identify truly unsellable Quest items and binding.
    ...
    for bag = 0, NUM_BAG_SLOTS do
        ...
        for slot = 1, numSlots do
            ...
            if (link and Atr_IsItemSellableOnAH(bag, slot, link, quality)) then
                ...
                if (classIdx and classIdx > 0) then
...
function Atr_Idle(self, elapsed)
    ...
    -- This prevents stalls where nothing progresses until the user interacts (e.g., scrolls).
    ...
    if (gCurrentPane and gCurrentPane.activeSearch and gAtr_ptime) then
        ...
        if (s.processing_state == KM_POSTQUERY and s.query_sent_when and (now - s.query_sent_when) > 10) then
